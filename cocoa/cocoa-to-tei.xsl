...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template name="main">
	...
	<xsl:if test="$debug='true'">
	  <xsl:result-document href="{$input}.pass0">
	...
	<xsl:if test="$debug='true'">
	  <xsl:result-document href="{$input}.pass1">
...
<!-- first pass -->  
  ...
  <xsl:template match="body">
    ...
    <xsl:for-each-group select="*|text()"
			group-starting-with="milestone[@level='1']">      
      <xsl:choose>
  ...
  <xsl:template name="group-by-section">
	...
	<xsl:value-of select="@value"/>
      ...
      <xsl:for-each-group select="current-group() except ."
			  group-starting-with="milestone[@level=$next]">
	<xsl:choose>
  ...
  <xsl:template name="maintext">
    <xsl:for-each-group select="current-group() except ." 
			group-starting-with="milestone[not(@level)]">
	<xsl:choose>
...
<!-- pass 2 -->
  ...
  <xsl:template match="ab[parent::p]"  mode="pass2">
      <xsl:apply-templates
	  select="@*|*|text()|comment()|processing-instruction" mode="pass2"/>
      <lb/>
  </xsl:template>
