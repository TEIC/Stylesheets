...
<xsl:stylesheet 
  ...
  <xsl:template name="expander">
    ...
    <!-- direct report -->
      <xsl:if test="key('ELEMENTS',@name)">
	<xsl:value-of   select="($element,@name,$distance,'element')"   separator=","/>
	<xsl:text>&#10;</xsl:text>
      </xsl:if>
	  
    <!-- report via macro -->
    <xsl:for-each select="key('MACROS',@name)">
      <xsl:value-of   select="($element,@ident,$distance,'macro')"   separator=","/>
      <xsl:text>&#10;</xsl:text>
      <xsl:variable name="element" select="@ident"/>
      <xsl:for-each select="content//rng:ref">
	<xsl:call-template name="expander">
    ...
    <xsl:for-each select="key('CLASSES',@name)">
      <xsl:value-of   select="($element,@ident,$distance,'model')"   separator=","/>
      <xsl:text>&#10;</xsl:text>
      <xsl:variable name="element" select="@ident"/>
  ...
  <xsl:template name="expandModel">
    ...
    <xsl:for-each select="key('MEMBERS',@ident)">
      <xsl:value-of   select="($element,@ident,$distance,'class')"   separator=","/>
      <xsl:text>&#10;</xsl:text>
    </xsl:for-each>
  </xsl:template>
